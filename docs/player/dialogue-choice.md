...
在探索状态走近 NPC 或剧情触发点，按 `E`。屏幕切到对话界面：立绘或半身像、说话人名字、台词逐句或整段显示。
...
1. 走近一个 NPC（比如土地庙外的李天狗），看互动提示出现，按 `E` 进入对话。
...
选项一旦选定，游戏内没有「悔棋」按钮——但你可以在关键对话前按 `Esc` 存进一个槽，选完看看效果不满意，再读档换一条路线试试（见 [存档与设置](./save)）。这在雾津是正当玩法，别有心理负担。
...
**选项都是灰的，是不是卡关了？**
先看灰色选项旁有没有提示——通常是缺某条规矩、某件物品，或任务进度没到。回去补条件，回来再选。
