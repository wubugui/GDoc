...
主编辑器**改不了背景图列表**——背景与正典深度优先在 **[角色照明实验室](../editors/render-domain/character-lighting-lab)** 维护并导出；旧 [场景深度工具](../editors/render-domain/scene-depth-editor) 仅作对照。若画面缺层或遮挡不对，改完导出后再回场景面板验证出生点与实体位置。
