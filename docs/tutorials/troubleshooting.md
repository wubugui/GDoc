...
- **担心保存丢内容**：危险区的真假区分，哪些是真要当心的、哪些其实没事
...
先别慌——编辑器整体相当稳，**绝大多数情况下这些内容都不会丢**：编辑器保留它不编辑的字段，早期「保存会整段重建、抹掉手写内容」的说法基本是过时的旧框架。真正要留意的，只有下面这**三类**。
...
下面这些以前的文档写成"会丢"，逐条核实源码后确认**都不成立**：

| 曾经的说法 | 实情 |
|---|---|
| 热区内部详情、观察说明 | 不丢，编辑器保留不认识的字段 |
| NPC 巡逻路线、出生点 | 不丢，同样保留未知键 |
| 打开并保存过的图对话节点 | 不丢，已知字段全保留，未知类型整块透传 |
| 音频每条 | 不丢，保存时复制原条目全部字段，只更新音源路径（但面板本身没有音量/循环的输入口，这两项要去场景/动作/过场配） |
| 部分过场步骤 | 不丢，往返无损 |
| 档案·人物簿印象（impressions） | 能删单条，每条都有删除按钮 |
| 档案·书籍的页（Page） | 能删，界面就有删除入口 |
| 档案切换子 Tab | 有自动提交保护，不会因为没先 Apply 就丢内容 |
| 物品的动态描述 | 能删单条，每条都有删除按钮 |

### 怎么用这页判断

1. **先查上面第三类**——十有八九你担心的其实不会丢
2. 真编辑不到的字段，对照第一类找到该去的专项工具
3. 踩到第二类里明确的破坏性操作，那是设计使然，不是 bug
4. 拿不准就去 [危险区](../editors/concepts/danger-zone) 或 [可编辑面参考](../reference/authoring-surface) 核对逐面板清单
5. 重要文案还是建议用版本管理，大改前先备份或提交一版，图个安心

### 雾津例子

给调查热区手写了一大段说明，回来一看还在——不是危险区的坑；真正要小心的是把区域改成深度地板，或者用场景面板的「重新导入背景」换了一张多层背景图。
