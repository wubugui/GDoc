...
| 部分过场步骤 | 面板外的字段会丢 |
| 档案·人物簿印象（impressions） | 只能加新条、不能删单条，写错了只能新增覆盖 |
| 档案·书籍的页（Page） | 界面不提供删除，规划页数要保守 |
| 档案切换子 Tab（人物/见闻/文档/书籍） | 未 Apply 就切走，正在编辑的内容直接丢，不弹提示 |
| 转盘/扎纸的专家 JSON、裸输入字段 | 格式不合法会导致**整段保存失败**，动手前先备份 |
| 物品的动态描述 | 只能加不能删单条 |
