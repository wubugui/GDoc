...
- **人物簿的印象（impressions）**是一组「条件 + 一段文字」，随剧情推进逐条追加，玩家会看到印象越写越厚。**这里只能加新条、不能删旧条**——写错了没法直接删，只能再加一条去覆盖观感。下笔前先想好推进顺序（比如「初见→熟识→知晓身世」），一次性按顺序写完，能省不少返工。
- **已知信息**同样是条件加文字的分段列表，适合拆成「外貌」「脾气」「来历」这种小节，每段各自的解锁条件互不影响。
- **书籍**是书（Book）→ 页（Page）→ 条（Entry）三级结构，适合装线装古卷这类需要「翻页」体验的长文。**页（Page）这一层界面不提供删除**，规划书籍结构时页数要保守；条目（Entry）层面可以放开手脚增删。
- **切换人物/见闻/文档/书籍这几个子 Tab 之前必须先 Apply**——正在编辑的条目没保存就切走，改动会直接丢，不会弹提示。
- 每条见闻/文档最好配上**解锁条件**和**首次阅读动作**：前者决定玩家什么时候能翻到，后者决定「读过了」这件事有没有在游戏里留痕迹（比如记一个旗标）。首次阅读动作漏填，等于玩家读了但游戏当他没读过。

### 音频：登记简单，持久化有限

- 面板保存时**每条只写源文件路径**——你若指望在这里长期调音量、设循环次数，存不住，保存后会被抹掉。要控制音量/循环，走场景、动作或过场自己的播放设置，而不是硬塞进音频条目。
...
| 边界 | 说明 |
|---|---|
| **音频条目保存只留源文件** | 手写的音量、循环等字段会被抹掉，别指望面板能长期存住这些 |
| **档案印象只能加不能删单条** | 填错的条目会一直占坑，需要新增一条覆盖，或找程序清理 |
| **书籍的 Page 不能删** | 界面限制，规划书籍结构时页数要保守 |
| **切换档案子 Tab 未 Apply 会丢** | 人物/见闻/文档/书籍互相切换前必须先保存 |
| **插图按钮只有档案有** | 别的富文本框想插图只能手打标签，正经配图请在档案里做 |
