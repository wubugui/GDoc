...
物品面板里「随状态变化的描述」——加一条可以，删某一条不行。改描述用「改最后一条」或整体规划，别指望逐条擦掉。这是物品面板最容易踩的坑，第三节会细讲。
:::
...
- **动态描述列表**：每条 = 一个触发条件 + 对应文本，用来做「鉴定前/鉴定后」「白天看/夜里看」这类描述随情境变化的效果。**这个列表只能新增条目、不能删除单条**——写错一条别想着删掉重写，只能再加一条优先级更高/更正确的，或者交给程序整体处理这条物品数据。多条命中时具体展示哪条以游戏内判定逻辑为准，务必逐条在预览里实测。
...
- 商店面板结构简单，重建风险很低，日常调价、上下架都可以放心改。
...
- **物品的动态描述列表只能新增、不能删除单条**——这是本页最重要的边界提醒，写描述前先想清楚顺序和条件，别指望能反悔删除，见上文进阶部分。
- **改物品 id** 会让所有引用它的商店、拾取热区、任务条件、富文本标签全部失效，等同于做一次迁移，操作前先全局搜索确认引用范围。
- **删除物品或商店前**，要确认没有场景热区、任务条件、对话动作还在引用它——删除后引用处会指向不存在的目标。
- 更系统的「哪里改了会丢、哪里编辑器根本够不着」参考 [危险区](../editors/concepts/danger-zone)。
...
| 现象 | 原因 | 怎么办 |
|---|---|---|
| 动态描述写错了删不掉 | 面板界面只支持新增，不支持删除单条 | 加一条优先级更高/更正确的条目覆盖表现，或找程序整体处理 |
