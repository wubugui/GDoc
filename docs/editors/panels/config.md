...
以下两类字段是这一页**真正的盲区**——本页检视器完全没有入口，不管你在这个面板里怎么翻都改不到，需要去别的地方。**改不到不等于会丢**：这两类数据都不会因为你在全局配置页保存而被清掉或覆盖，只是这里给不了你编辑的入口：

| 字段 | 在哪儿改 |
|---|---|
| **主角外观/动画** | 全局配置页够不着，去[玩家化身](./avatar)专用面板改——这部分数据本来就存在别处，跟全局配置是两回事，不会因为你保存全局配置而受影响 |
| **像素密度匹配相关的一两个开关** | 决定角色贴图是否按背景的像素密度做匹配处理（避免像素画风的人物和背景缩放观感不一致），主编辑器完全没有可视化入口；运行预览里的调试菜单可以临时切换看效果，但那只是**临时调试**，不会写回配置文件，重启就恢复原状——这个开关本身不会因为你保存全局配置而被清掉或改动，只是没有可视化入口去设置它；真要长期改，需要按危险区流程升级或手改文件，别在这个面板里死磕找入口 |
...
| 主角外观/动画改不动 | 这项在本页是盲区，但不会因为保存全局配置而丢 | 去[玩家化身](./avatar)面板改 |
| 像素缩放观感不对，怎么都改不了 | 相关开关不在本页 GUI 范围内，保存全局配置不会动到它 | 用运行预览调试菜单临时验证效果，长期改动按危险区流程处理 |
