...
![任务面板](/img/screenshots/panel-quest.png)

*任务面板：左树是分组与任务，中间关系图看先后与支线。*
...
- **旧式的单字段"下一任务 id"已废弃**：任务先后关系现在都用下一环边表达，面板保存时会清掉这个旧字段，别在别处手写它。
...
| 手写的旧"下一任务 id"没生效 | 该字段已废弃，保存时会被清理 | 改用下一环边 |
