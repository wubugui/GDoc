...
玩家背包里的湿鞋、符纸、铜钱——都要先在**物品面板**立档：id、显示名、类型、静态描述、最大堆叠数、参考买入价，以及**按条件变的描述**（玩家满足某个[条件](../concepts/conditions)时，背包里显示另一段说明）。[商店](./shop)卖你、场景热区拾取引用你、[富文本](../concepts/rich-text)里的物品标签也认这些 id。读完这页你能建出一件「会随剧情变说明」的物品，并搞清楚这个面板最有名的一个坑——动态描述**只能加不能删**。
...
| **删掉单条动态描述** | **不能** —— 界面上**只能添加，不能删除**已有的一条 |

这是物品面板最容易踩的坑：写错一条动态描述之后，**不要指望点垃圾桶就能删掉它**——这个操作在界面上不存在。补救方式只有两种：

1. **再加一条更优先、条件更精确的动态描述**，把错的那条实际上「盖过去」——但错的那条数据仍然留在物品里，只是不会被命中而已。
2. 找程序或做更深层数据处理，把整件物品的数据重新梳理一遍。

所以写动态描述前最好先想清楚条件和文本，宁可多花一分钟打草稿，也别指望「填错了大不了删掉重填」。
...
- **动态描述只能加不能删**：这是本面板最著名的限制，上面已经讲透，动手前务必想清楚每一条要不要加。
...
| 现象 | 原因 | 怎么办 |
|---|---|---|
| 动态描述写错了，删不掉 | 面板本身只支持添加，不支持删除单条 | 加一条更精确、优先命中的动态描述覆盖效果，或找程序做整体数据清理 |
