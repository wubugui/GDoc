...
还有一个容易踩的坑：全局配置里本该有个"初始用哪套化身"的设置项，但它平时常常是在**独立编辑器**里维护的，主配置页面上可能根本碰不到这个字段——改主角相关的东西，优先来这个面板找，别在全局配置里干等。
...
全局配置页面理论上有一个"初始使用哪套化身"的设置项，但实际维护它的常常是一个独立的编辑器，主配置界面自己**可能碰不到这个字段**——这时候不要在全局配置里死磕，直接来玩家化身面板本身操作，这里才是权威入口。类似地，如果项目里有"实体像素密度匹配"这类底层缩放参数，主配置页面同样可能是盲区，遇到缩放不对的问题要及时找程序确认，而不是自己瞎调别的字段去凑效果。
...
- 和[全局配置](./config)配合：理论上的"初始化身"引用在那边，但常是盲区，实际改动仍以本面板为准。
...
| 边界 | 说明 |
|---|---|
| **全局配置里的初始化身可能是盲区** | 主配置页面碰不到时，以本面板为准，不要在配置页死磕 |
| **像素密度类底层参数可能也是盲区** | 缩放表现不对时，先确认是不是这一类字段导致，找程序核实 |
