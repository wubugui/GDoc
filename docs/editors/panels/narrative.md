...
- **从 / 到**：只能在画布**连线**模式里改，检视器里只读——拖错线就切回连线模式重连。
...
| 风险 | 说明 |
|---|---|
| 迁移端点只读 | 改错 from/to 必须切到连线模式重新拉线 |
| 旧跨图端点 | 历史遗留的跨图连接可能编不了，需人工清理或找程序处理 |
| 状态里的隐藏信息无界面 | 运行时如果状态上带着编辑器不认识的附加信息，GUI 维护不到——落到这类字段就是[危险区](../concepts/danger-zone)里的"盲区"，不要自己手写凑数 |
...
| 现象 | 原因 | 怎么办 |
|---|---|---|
| 迁移连错了起点终点 | from/to 只在连线模式改 | 切连线模式重新拉线 |
