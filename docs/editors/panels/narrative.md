...
- **从 / 到**：检视器的结构化表单里这两项是**只读**的，正常改法是切回画布**连线**模式重新拉线，最不容易手滑。检视器里还有一个**「高级 JSON」**页，把这条迁移的原始数据摊开给你看，需要的话可以在那页直接改起止点，但这条路更容易改错格式——一般情况优先用连线模式，「高级 JSON」留给老手当旁路用。
...
给一个信号、一个状态或一张图改名字，面板里走的是"重构"流程，不用你自己翻遍全项目找引用：
...
| 风险 | 说明 |
|---|---|
| 迁移端点只读 | 结构化表单里 from/to 只读，正常改法是切到连线模式重新拉线；检视器的「高级 JSON」页也能直接改这两个值，是给老手用的旁路 |
| 旧跨图端点 | 历史遗留的跨图连接可能编不了，需人工清理或找程序处理 |
| 状态里的隐藏信息，结构化表单没有对应的框 | 运行时如果状态上带着编辑器不认识的附加信息，结构化表单不给它留输入框；检视器的「高级 JSON」页能看到并直接改这块原始数据，需要动它就去那一页手改 |
