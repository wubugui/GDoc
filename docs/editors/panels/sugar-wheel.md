...
- 旧字段「最多同时显示几句气泡」类的设置，是**已经废弃的旧字段**——现在的围观氛围逻辑不再读这个值，运行时不会用它，就算数据里还留着也不起作用。编辑器保存时把它顺手清掉，是**清理这类死字段**，不是把你的配置弄丢，不用因为它"消失"就慌。
...
- 「最多同时显示几句气泡」一类的旧字段是**已经废弃、运行时不再使用的死字段**，保存时编辑器会顺手清掉——这是清理废弃字段，不是丢你的配置，别把它误当成"数据丢失"来提防。
...
| 气泡最大显示数设置保存后没了 | 这是已废弃的旧字段，运行时不再使用，编辑器保存时顺手清掉，不算丢数据 | 不用管它，围观氛围靠「旋转氛围脚本」那套配置生效 |
