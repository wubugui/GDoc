...
![剧本面板](/img/screenshots/panel-scenarios.png)

*剧本面板：右侧配阶段、进线条件、暴露旗标。*
...
有一个字段叫结局标记，记录这个阶段"具体是怎么结束的"（比如"渡口"这一拍是靠成功捞到鞋结束、还是靠别的分支结束）——但编辑器**没有界面能填它**。正常改别的字段、Apply，它会原样保留；**给这个阶段改名字**时新名字不会继承旧名字下的结局标记，等于要重新记一份。需要记录"阶段是怎么完成的"这类信息，日常协作更省心的做法是单独设一个旗标来代替。
...
- **phase 的结局标记没有编辑控件**，普通保存会原样保留它；**只有给这个阶段改名字**，新名字才不会继承旧名字下的结局标记值。
