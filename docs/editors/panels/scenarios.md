...
### phase 的结局标记（这项界面没有编辑控件）

有一个字段叫结局标记，理论上可以记录这个阶段"具体是怎么结束的"（比如"渡口"这一拍是靠成功捞到鞋结束、还是靠别的分支结束）——但编辑器目前**没有任何界面能填它**。不过不用担心普通保存会把它冲掉：**这个值有专门的防丢代码兜底、还有回归测试锁定这个行为，正常打开面板、改别的字段、Apply，它会原样带下去，不会无声消失。** 唯一真的会让它"跟丢"的情况是**给这个阶段改名字**——换了名字之后，新名字不会自动继承旧名字下的结局标记，等于要重新记一份。如果设计上确实需要记录"阶段是怎么完成的"这类信息，由于界面填不了它，日常协作里更省心的做法还是用别的机制（比如单独设一个旗标）来代替。
...
- **phase 的结局标记没有编辑控件**，但普通保存不会丢它——有专门的防丢代码和回归测试锁定；**只有给这个阶段改名字**，新名字才不会继承旧名字下的结局标记值。
...
| 给阶段改名后，结局标记好像没跟上 | 改名不会让新名字继承旧名字下的结局标记值，这是唯一会丢的场景，普通保存不受影响 | 改名前记好原值；日常记录"阶段怎么完成"更推荐用旗标等别的机制 |
