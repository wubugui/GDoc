...
- **是否恢复状态**：播完是否还原玩家进过场前的某些状态（比如过场里临时把玩家挪了个姿势，播完要不要恢复原样）；老数据里的过时字段保存时会被顺手清掉（不是丢数据，是运行时已经不用的旧格式），以检视器里当前的项为准。
...
呈现步骤是**数据无损**的：像插图显示的缓推缓移运镜、字幕的表情标记这类比较复杂的字段，都有**专属的编辑区域**来填，保存时认得也保留；碰到编辑器**不认识的呈现步骤类型**，也是**整块原样透传**，不会丢东西。

### 废弃字段清理（不是丢数据）

- 过场顶层的旧版「命令列表」写法，保存时会被清掉——现在统一用「步骤」表达，旧写法运行时已经不用了，清掉不影响游戏。

### 设计限制（不是数据丢失）

- 逻辑步（跑动作）能用的动作，只是全部动作里大约三十来种的一个子集，覆盖最常见的发东西/改状态——这是过场这里**故意做的限制**，不在这个子集里的类型，编辑器会直接**拒绝保存**，不是"丢"，是根本存不进去。

### 操作提醒

- **目标场景/出生点写错**：不是编辑器丢数据，是配错了目标——播完会掉进黑屏或落墙里，保存前核对一遍场景和出生点 key 是否真的存在。
