...
![过场面板](/img/screenshots/panel-cutscene.png)

*过场面板：左列过场，右侧时间线步骤与落点。*

- **过场身份**：内部 id、播完落在哪张场景、哪个出生点、落点坐标——**目标出生点**和**落点 x/y** 是两种精确度不同的选择：填出生点就沿用那张场景已经登记好的位置，填精确 x/y 是「不管出生点，直接把玩家钉在这个坐标」，两者按需二选一或搭配兜底。
- **是否恢复状态**：播完是否还原玩家进过场前的某些状态（比如过场里临时把玩家挪了个姿势，播完要不要恢复原样）；老数据里的过时字段保存时会被顺手清掉，以检视器里当前的项为准。
...
呈现步骤是**数据无损**的：像插图显示的缓推缓移运镜、字幕的表情标记这类比较复杂的字段，都有**专属的编辑区域**来填，保存时认得也保留；碰到编辑器**不认识的呈现步骤类型**，也是**整块原样透传**。

### 废弃字段清理

- 过场顶层的旧版「命令列表」写法，保存时会被清掉——现在统一用「步骤」表达。

### 设计限制

- 逻辑步（跑动作）能用的动作，只是全部动作里大约三十来种的一个子集，覆盖最常见的发东西/改状态——不在这个子集里的类型，编辑器会直接**拒绝保存**。
