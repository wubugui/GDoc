...
这三个数字**互相联动、永远保持原图比例**——填其中任何一个，另外两个自动更新。工具没有「强行拉伸变形」这个选项，图片不会被拉扁；如果你确实需要非等比的裁切效果，那不是这个工具的职责范围，需要用其它图像处理手段单独处理。
...
- **镜像后先别急着定稿**：游戏里角色的落地点是按脚底对齐的，镜像翻转不影响这个对齐；如果角色本身设计不对称，导出后建议回 [动画预览](./anim-preview) 或直接 F5 进场景看一眼，确认视觉效果符合预期。
...
**Q：镜像之后，游戏里角色的走位会不会乱？**
不会，镜像只翻转画面本身，脚底锚点位置不变；如果角色本身设计不对称，导出后建议回动画预览或 F5 场景里实际看一眼确认效果。
