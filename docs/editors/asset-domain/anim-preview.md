...
动画浏览像是货架上贴的小标签——扫一眼知道「有这个状态、名字这么拼」；动画预览则像是把这件戏服请上真人模特台，打上和舞台一样的灯光、慢镜头走一遍每个动作，专门用来挑毛病：脚有没有在地上打滑、动作循环起来接口顺不顺、光打上去质感对不对。它自动发现工程里所有已经产出的动画包，不需要你手动导入或指定路径；产出目录里一有新包出现，列表也会自己更新，不用你重开工具刷新页面。

它是一面**只读的镜子**——能让你看清楚问题在哪，但改不了动画本身。发现问题之后，回 [视频转图集](./video-to-atlas) 或 [动画后处理](./animation-pipeline) 修源重导，这里管的只是「验」这一步。
