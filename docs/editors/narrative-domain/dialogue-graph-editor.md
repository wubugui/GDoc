...
「哟，寻狗啊？」接选项「打听码头怪事」「算了」——分支一多，表格就盯不住谁接谁。**图对话编辑器**用**节点 + 连线**画整张对话：台词、选项、条件分支、跑动作、结束，一眼就能看出整张图的走向。雾津里关二狗的码头闲聊、城隍庙门口的抉择、旗标门控的隐藏选项，都适合在这里编。读完这页你能：认清「独立窗」和「面板」其实是同一份数据，把七种节点的每一个填法搞懂，也知道保存时哪些节点会被整段重写。
...
老手窍门：纯粹挪动节点在画布上的位置，不会触发保存时的重建风险；只有真的改了连线目标（出口指向变了），保存时相关节点才会被重新整理。所以想美化布局、腾地方，尽管拖，不用担心「拖一下就丢字段」。
...
| 当心 | 说明 |
|---|---|
| **重建区：打开并保存过的节点会被整段重建** | 你在某个节点上多塞的、界面认不出的字段，只要打开过这个节点并保存，**会丢**；从没在编辑器里打开过的节点原样保留——别赌运气，只填面板里真有的框 |
| 图级前置里的非结构化叶子 | 某些特殊写法会被单独保留，与节点重建规则不同，但仍以检视器展示为准，不确定就不要手写花样写法 |
