...
:::info[更完整的对照表]
全工具用途、形态与文档状态，见 **[工具速查表](./tool-matrix)**。参考区 **[工具打开方式速查](../reference/open-tools)** 将陆续补齐与本文同步的内容。
:::
...
控制台页上一排工具按钮，与 `./dev.sh` 任务名一一对应（见上表）。另外提供：
...
| 现象 | 怎么办 |
|---|---|
| 提示 Python 环境缺失 | 在游戏仓库根目录跑 `./bootstrap.sh` |
| 任务名报错 | 对照上表，注意连字符（如 `asset-ingest`） |
| Web 工具起不来 | 确认 Node 版本 ≥ 20，看终端里实际端口 |
| 点了没反应 | 看终端是否报错；必要时关掉旧窗口再开 |

更多排查 → **[教程：出问题怎么办](../tutorials/troubleshooting)**。

---

## 接下来

- **[编辑器总览](./overview)** —— 两扇门与工具分类
- **[工具速查表](./tool-matrix)** —— 全工具一览
- **[Web 控制台](./concepts/web-console)** —— 仪表盘用法
