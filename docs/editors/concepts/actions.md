...
- **动作列表所在的父条目不用太担心手写内容被清空**：编辑器会保留它不认识的字段，热区的「观察」详情、图对话节点、过场步骤本身都是往返无损，不会因为打开保存过就被清掉。真正**编辑器完全没入口**、得换专项工具或手改的，只有旗标的「迁移/运行时」块、实体像素密度匹配这类盲区，见 **[危险区](./danger-zone)**。日常还是优先只通过动作编辑器填参数，界面之外的手写内容毕竟没有校验，容易打错字。
...
编辑器现在通常会保留它不认识的字段，所以大概率不是保存把内容洗掉了。先确认是不是引用的 NPC / 物品 / 场景本身被删了或改了 id；如果是真被清空的那种情况，多半属于**盲区**字段（旗标迁移块、像素密度匹配这类）或者你踩到了某个明确的破坏性操作（比如把区域从「标准」改成了「深度地板」），查 **[危险区](./danger-zone)** 对照确认，别默认是保存机制在整段重写。
