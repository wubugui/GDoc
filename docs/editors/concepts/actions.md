...
- **动作列表所在的父条目，若落在 [危险区](./danger-zone) 的重建范围，保存时只保留编辑器认识的字段**。只通过动作编辑器填参数，别手写塞额外键——例如热区的「观察」详情、被打开编辑过的图对话节点、过场步骤本身，都是保存即重写的重建区。
...
大概率是这条动作所在的父条目落在了重建区——你手改过的、面板没提供入口的字段，保存时被整段重写掉了。查 **[危险区](./danger-zone)** 确认这块是不是重建区，之后只通过面板填参数。
