...
关二狗肯不肯开口、城隍庙侧门是否解锁、遭遇里「硬闯」选项亮不亮——都取决于 **条件**：当前游戏状态**是否满足**你的要求。条件和动作是一对搭档：**条件**决定「能不能 / 要不要」，**动作**决定「发生什么」，见 **[怎么编排动作](./actions)**。读完这一页，你能看懂条件树的七种「查什么」、三种「怎么组合」，以及它在各面板里出现的所有位置。
...
| **活计计数** | 某条可重复活计累计完成次数 | 选活计图 / 结算出口；选比较符；填次数 | 「背尸送义庄做过 ≥ 2 单」才开新闲话 |
| **激活位面** | 当前激活位面是否等于某 id | 选位面；「不是现世」用取反包一层 | 「鬼打墙位面激活时」才显示某热区 |

拼写必须与游戏里登记的一致——旗标名、任务 id、位面 id 这些要跟对应面板里登记的名字对得上。**编辑器不一定当场报错**，写错了通常会**静默不成立**（条件永远判否），保存前务必用 `F5` 预览验证一遍。
...
- **结构化条件树**：和上面七种叶子完全一样，功能最全，能表达任意复杂逻辑。
...
- **旗标类叶子的比较值** 可以插富文本引用（`[tag:flag:…]` 等），让条件判断和动态文案保持同一份数据源，不用两处各写一遍。
- **活计计数** 看的是历史完成次数，不是「当前是否激活」；当前激活哪一单，用任务状态或旗标另判。
- **激活位面** 判的是当下生效的位面（含手动覆盖压过叙事点名后的结果）；正式切换优先走叙事状态机。
...
- 条件挂在父条目上时是往返无损的——编辑器会保留它不认识的字段，不会被清空。真正编辑器完全没入口、得换专项工具或手改的，只有旗标的「迁移/运行时」块这类盲区，见 **[危险区](./danger-zone)**。日常还是优先只通过条件编辑器操作，手写额外字段没有校验，容易打错。
