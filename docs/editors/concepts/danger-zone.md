...
- **场景的多层背景、深度 / 遮挡碰撞主体** → 正典深度与角色照明导出以 **[角色照明实验室](../render-domain/character-lighting-lab)** 为主；旧 **[场景深度工具](../render-domain/scene-depth-editor)** 仅作对照。改背景 / 深度 / 曝光后要重烘并重新导出照明，否则游戏哈希门会禁用烘焙。原理见 **[伪世界角色照明](../../dev/character-lighting)**。
...
- **把区域从「标准」改成「深度地板」** → 会清掉它的进入 / 停留 / 离开动作（还有气味）；界面会先确认。深度地板区本就不触发这些，正常用没事；但**切过去再切回来，原来的动作找不回**。
- **把区域从「深度地板」切回「标准」** → 会丢掉深度地板专属的地板偏移加成；同样有确认。
...
它会清掉这个区域的进入 / 停留 / 离开动作和气味。如果这个区域原本挂了剧情触发，切之前先确认不再需要，或把触发挪到别的标准区域。切回标准还会丢掉深度地板专属的地板偏移加成。

**标准区域的气味还能不能改？**

能。场景面板的标准区域已有气味配置入口；切成深度地板后气味会被清掉并禁用编辑。
...
- **[主编辑器总览](../main-editor/overview)** —— 31 块面板入口
