...
| `npm run test:godot-fade-visuals` | 只扫黑场淡入淡出关键帧视觉门禁 |
...
A:`./dev.sh pull` 同步代码 + 运行时大文件;`./scripts/pull-all.sh` 等价于 `./dev.sh pull --editor`,会额外拉编辑器工程资源。只玩/改内容用前者即可;要动编辑器工程侧资源时用后者。
